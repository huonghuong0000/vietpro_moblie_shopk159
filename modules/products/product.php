...
$totalRow = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM comment WHERE prd_id=$prd_id"));
...
href="index.php?page_layout=product&prd_id='.$prd_id.'&page='.$prePage.'">&laquo;</a></li>';
...
for($i = 1; $i <= $totalPage; $i++){
    ...
    href="index.php?page_layout=product&prd_id='.$prd_id.'&page='.$i.'">'.$i.'</a></li>';
}
...
$listPage .= '<li class="page-item"><a class="page-link" 
href="index.php?page_layout=product&prd_id='.$prd_id.'&page='.$nextPage.'">&raquo;</a></li>';
//
...
<div id="product">
        ...
        <div id="product-details" class="col-lg-6 col-md-6 col-sm-12">
            ...
            <ul>
                ...
                <li id="status" class="<?php if($row['prd_status']==0){echo 'text-danger';} ?>">
                    <?php if($row['prd_status']==0){echo 'Hết hàng';}else{echo 'Còn hàng';} ?>
                </li>
            </ul>
    ...
    if(isset($_POST['sbm'])){
        ...
        $sql = "INSERT INTO comment(comm_name, comm_mail, comm_date,
                comm_details, prd_id) VALUES('$comm_name',
                '$comm_mail','$comm_date','$comm_details','$prd_id')";
    ...
    <!--	Comments List	-->
    <?php  
    $sql = "SELECT * FROM comment WHERE prd_id=$prd_id ORDER BY comm_id DESC LIMIT $keyStart,$rowPerPage";
    $query = mysqli_query($conn, $sql);
    ?>
